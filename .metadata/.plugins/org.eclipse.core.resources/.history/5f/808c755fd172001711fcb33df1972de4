...
public class Solution {
	...
	the values of the B array elements, and then A_current.
	*/
	...
	public static int getNextPrime(int currentPrime) throws Exception {
		...
		if (currentPrime < 2) {
			...
			while (!primeFound) {
				...
				primeFound = true;
				for (int factorIndex = 2; factorIndex <= rootValue; factorIndex++) {
    ...
    public static void main(String[] args) {
        ...
        ArrayList<Stack<Integer>> bStackArray = new ArrayList<Stack<Integer>>();
        
        for (int iteration = 0; iteration <= q; iteration++) {
        	bStackArray.add(new Stack<Integer>());
        }
        ...
        for (int iteration = 1; iteration <= q; iteration++) {
        	...
        	while (!aCurrent.empty()) {
        		...
        		if (plateValue % currentPrime == 0) {
        			bStackArray.get(iteration).push(plateValue);
        		} else {
        ...
        for (int bIndex = 1; bIndex <= q; bIndex++) {
        	Stack<Integer> bStackElement = bStackArray.get(bIndex);
        	while (bStackElement.size() > 0) {
        		System.out.println(bStackElement.pop());
        	}
        }
        while (aCurrent.size() > 0) {
